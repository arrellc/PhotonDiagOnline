...
TOF_drift_queue = deque(maxlen=runAvgWindow) 
...
# ---- Start processing
with source(channels=[streaked, reference, YAGDiode, event_codes, BAM1, I0, delay]) as stream:
    diode_pumped = 1
    diode_unpumped = 1
    I0Val = 1
    ...
    while True:
        ...
        # Read values from received message
            ...
            try:
                ...
                print ('Type Error wf_str type was ',type(message.data.data[streaked]))
                continue
            ...
            if PulseID % 24 == 0:
                ...
                if abs(EnergyRBpv.get()-E_centre) < E_range and wf_str.sum()>4.5:
                ...
                try:
                    ...
                    print('Variable not definded - likely FEL energy out off feedback range')
